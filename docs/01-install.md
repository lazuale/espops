...
Если IP `10.0.2.7` не найден, Docker не сможет привязать порты к этому адресу. Нужно либо назначить IP серверу, либо поменять `ESPOCRM_BIND_IP` в `.env.prod` и `.env.dev`.
...
|---|---|---|
| `ESPOCRM_BIND_IP` | `10.0.2.7` | IP сервера, на который Docker публикует порты |
